...
const get_expense_subcat = async (req,res)=>{
    try {
       const data =  await ExpenseSubCategory.find({ }).sort( { _id : -1 } ).skip(req.body.last_id).limit(10).lean()
        if(data){
...
const search_expense_subcat = async (req,res)=>{
    try {
       const data =  await ExpenseSubCategory.find({$or:[{subCatName: {$regex : new RegExp(req?.body?.search),$options:'i'}},{catName: {$regex : new RegExp(req?.body?.search),$options:'i'}}]}).lean()
        if(data){
            if(req?.body?.last_id == 0){
