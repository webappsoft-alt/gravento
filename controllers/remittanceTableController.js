    ...
    try {
        ...
        if (data) {
            for (const element of data) {
                const remittanceVoucher = await RemittanceVoucher.find({ remittanceNumId: element._id });
                const data2 = { data: element, remittanceData: remittanceVoucher };
                arr.push(data2);
            }
            if (req?.body?.last_id == 0) {
                const data1 = await RemittanceTable.find({}).count();
                res.status(200).json({ data: data, dataRem: arr, count: data1 });
            } else
                res.status(200).json({ data: data, dataRem: arr, count:"" });
        } else {
            res.status(200).json({ data: [], dataRem: [], count: '' });
        }
    } catch (error) {
        res.status(400).send(error.message);
    }
...
const search_rem_Table = async (req, res) => {
    try {
        const data = await RemittanceTable.find({
            $or: [{ remittanceType: { $regex: new RegExp(req?.body?.search), $options: 'i' } }, { amount: { $regex: new RegExp(req?.body?.search), $options: 'i' } }
                , { remittanceTabledate: { $regex: new RegExp(req?.body?.search), $options: 'i' } }, { voucherNumber: { $regex: new RegExp(req?.body?.search), $options: 'i' } }]
        }).lean()
        if (data) {
            if (req?.body?.last_id == 0) {
